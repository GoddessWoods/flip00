...
traindf = data.drop(['id',"bone_length", "rotting_flesh", "hair_length", "has_soul"], axis = 1)
...
'''
rf_best = RandomForestClassifier(n_estimators = 50, criterion = 'entropy', max_depth = None, random_state = 0, max_features = 'auto')
bag_best = BaggingClassifier(max_samples = 10, n_estimators = 50, random_state = 0)
gbc_best = GradientBoostingClassifier(n_estimators = 10, max_depth = 3, learning_rate = 0.3, random_state = 0)
lr_best = LogisticRegression(C = 5, penalty = 'l2')
svc_best = SVC(C = 5, degree = 3, kernel = 'linear')
knc_best = KNeighborsClassifier(algorithm='auto', leaf_size=10, n_neighbors= 20, p=5, weights='distance')
model = XGBClassifier(nthread=4, learning_rate=0.9, n_estimators=50, max_depth=5, gamma=0,
                      subsample=0.9, colsample_bytree=0.5)
# voting
voting_clf = VotingClassifier(estimators = [('xgb',model),('rf', rf_best), ('bag', bag_best), ('gbc', gbc_best), ('lr', lr_best), ('svc', svc_best), ('knc',knc_best)],voting = 'hard')
